...
use creamui_core::{BoxedWidget, ColorToken, Size, StateStyle, Style, TextAlign};
...
fn main() {
    ...
    run(
        ...
        move |viewport: Size| -> BoxedWidget {
            ...
            let button_layout = LayoutStyle {
                size: fixed(260., 54.),
                ..Default::default()
            };
            // This value can be stored, cloned, and shared by every button
            // that should have the same visual language.
            let action_style = Style::new()
                .layout(button_layout.clone())
                .background(ColorToken::SurfaceElevated)
                .border(ColorToken::Border, 1.)
                .corner_radius(8.)
                .hover(
                    StateStyle::new()
                        .background(ColorToken::AccentHover)
                        .border(ColorToken::Accent, 2.)
                        .corner_radius(14.),
                )
                .pressed(
                    StateStyle::new()
                        .background(ColorToken::AccentPressed)
                        .border(ColorToken::SelectionText, 2.)
                        .corner_radius(4.),
                );
